...
function handleAxiosError(error: any, action: string, language?: string) {
  ...
  if (error?.response?.status) {
    ...
    switch (status) {
      case 401:
        throw new Error(
          getErrorMessage("errorAuth", lang, "Authentication error")
        );
      case 403:
      ...
      case 502:
        throw new Error(
          getErrorMessage("errorBadGateway", lang, "Bad gateway")
        );
      case 504:
        throw new Error(
          getErrorMessage("errorTimeout", lang, "Gateway timeout")
        );
      default:
        throw new Error(
          getErrorMessage("errorDefault", lang, "An error occurred")
        );
    }
  }
  throw new Error(
